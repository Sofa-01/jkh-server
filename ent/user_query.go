...
type UserQuery struct {
	...
	withRole              *RoleQuery
	withInspections       *TaskQuery
	withAssignedBuildings *BuildingQuery
	withAssignedUnits     *InspectorUnitQuery
...
// QueryInspections chains the current query on the "inspections" edge.
func (_q *UserQuery) QueryInspections() *TaskQuery {
...
// QueryAssignedBuildings chains the current query on the "assigned_buildings" edge.
func (_q *UserQuery) QueryAssignedBuildings() *BuildingQuery {
...
// QueryAssignedUnits chains the current query on the "assigned_units" edge.
func (_q *UserQuery) QueryAssignedUnits() *InspectorUnitQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withInspections:       _q.withInspections.Clone(),
		withAssignedBuildings: _q.withAssignedBuildings.Clone(),
		withAssignedUnits:     _q.withAssignedUnits.Clone(),
...
// WithInspections tells the query-builder to eager-load the nodes that are connected to
// the "inspections" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithInspections(opts ...func(*TaskQuery)) *UserQuery {
	query := (&TaskClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withInspections = query
	return _q
}

// WithAssignedBuildings tells the query-builder to eager-load the nodes that are connected to
// the "assigned_buildings" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithAssignedBuildings(opts ...func(*BuildingQuery)) *UserQuery {
	query := (&BuildingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAssignedBuildings = query
	return _q
}

// WithAssignedUnits tells the query-builder to eager-load the nodes that are connected to
// the "assigned_units" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithAssignedUnits(opts ...func(*InspectorUnitQuery)) *UserQuery {
	query := (&InspectorUnitClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAssignedUnits = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [4]bool{
			_q.withRole != nil,
			_q.withInspections != nil,
			_q.withAssignedBuildings != nil,
			_q.withAssignedUnits != nil,
		}
	)
	...
	if query := _q.withInspections; query != nil {
		if err := _q.loadInspections(ctx, query, nodes,
			func(n *User) { n.Edges.Inspections = []*Task{} },
			func(n *User, e *Task) { n.Edges.Inspections = append(n.Edges.Inspections, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAssignedBuildings; query != nil {
		if err := _q.loadAssignedBuildings(ctx, query, nodes,
			func(n *User) { n.Edges.AssignedBuildings = []*Building{} },
			func(n *User, e *Building) { n.Edges.AssignedBuildings = append(n.Edges.AssignedBuildings, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAssignedUnits; query != nil {
		if err := _q.loadAssignedUnits(ctx, query, nodes,
			func(n *User) { n.Edges.AssignedUnits = []*InspectorUnit{} },
			func(n *User, e *InspectorUnit) { n.Edges.AssignedUnits = append(n.Edges.AssignedUnits, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadInspections(ctx context.Context, query *TaskQuery, nodes []*User, init func(*User), assign func(*User, *Task)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(task.FieldInspectorID)
	}
	query.Where(predicate.Task(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.InspectionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadAssignedBuildings(ctx context.Context, query *BuildingQuery, nodes []*User, init func(*User), assign func(*User, *Building)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(building.FieldInspectorID)
	}
	query.Where(predicate.Building(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AssignedBuildingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadAssignedUnits(ctx context.Context, query *InspectorUnitQuery, nodes []*User, init func(*User), assign func(*User, *InspectorUnit)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(inspectorunit.FieldUserID)
	}
	query.Where(predicate.InspectorUnit(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AssignedUnitsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
