...
type TaskQuery struct {
	...
	withResults   *InspectionResultQuery
	withAct       *InspectionActQuery
...
func (_q *TaskQuery) QueryInspector() *UserQuery {
...
func (_q *TaskQuery) QueryBuilding() *BuildingQuery {
...
func (_q *TaskQuery) QueryChecklist() *ChecklistQuery {
...
func (_q *TaskQuery) QueryResults() *InspectionResultQuery {
...
func (_q *TaskQuery) QueryAct() *InspectionActQuery {
	query := (&InspectionActClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TaskQuery) WithInspector(opts ...func(*UserQuery)) *TaskQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *TaskQuery) WithBuilding(opts ...func(*BuildingQuery)) *TaskQuery {
	query := (&BuildingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) WithChecklist(opts ...func(*ChecklistQuery)) *TaskQuery {
	query := (&ChecklistClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) WithResults(opts ...func(*InspectionResultQuery)) *TaskQuery {
	query := (&InspectionResultClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) WithAct(opts ...func(*InspectionActQuery)) *TaskQuery {
	query := (&InspectionActClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := _q.withInspector; query != nil {
		if err := _q.loadInspector(ctx, query, nodes, nil,
			func(n *Task, e *User) { n.Edges.Inspector = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withBuilding; query != nil {
		if err := _q.loadBuilding(ctx, query, nodes, nil,
			func(n *Task, e *Building) { n.Edges.Building = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withChecklist; query != nil {
		if err := _q.loadChecklist(ctx, query, nodes, nil,
			func(n *Task, e *Checklist) { n.Edges.Checklist = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withResults; query != nil {
		if err := _q.loadResults(ctx, query, nodes,
			func(n *Task) { n.Edges.Results = []*InspectionResult{} },
			func(n *Task, e *InspectionResult) { n.Edges.Results = append(n.Edges.Results, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAct; query != nil {
		if err := _q.loadAct(ctx, query, nodes, nil,
			func(n *Task, e *InspectionAct) { n.Edges.Act = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TaskQuery) loadInspector(ctx context.Context, query *UserQuery, nodes []*Task, init func(*Task), assign func(*Task, *User)) error {
...
func (_q *TaskQuery) loadBuilding(ctx context.Context, query *BuildingQuery, nodes []*Task, init func(*Task), assign func(*Task, *Building)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		fk := nodes[i].BuildingID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "building_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TaskQuery) loadChecklist(ctx context.Context, query *ChecklistQuery, nodes []*Task, init func(*Task), assign func(*Task, *Checklist)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
...
func (_q *TaskQuery) loadResults(ctx context.Context, query *InspectionResultQuery, nodes []*Task, init func(*Task), assign func(*Task, *InspectionResult)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(inspectionresult.FieldTaskID)
	}
	query.Where(predicate.InspectionResult(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.ResultsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *TaskQuery) loadAct(ctx context.Context, query *InspectionActQuery, nodes []*Task, init func(*Task), assign func(*Task, *InspectionAct)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Task)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(inspectionact.FieldTaskID)
	}
	query.Where(predicate.InspectionAct(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.ActColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *TaskQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withInspector != nil {
			_spec.Node.AddColumnOnce(task.FieldInspectorID)
		}
		if _q.withBuilding != nil {
			_spec.Node.AddColumnOnce(task.FieldBuildingID)
		}
		if _q.withChecklist != nil {
			_spec.Node.AddColumnOnce(task.FieldChecklistID)
		}
	}
